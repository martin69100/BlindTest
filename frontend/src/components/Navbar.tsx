...
import { Music, Trophy, User as UserIcon, LogOut, Crown } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          {user ? (
            <div className="flex items-center space-x-2.5 sm:space-x-3">
              {/* Leaderboard Button (à côté du ELO) */}
              <button
                onClick={() => setIsLeaderboardOpen(true)}
                className="flex items-center space-x-1.5 px-3 py-1.5 rounded-full bg-dark-800/90 hover:bg-amber-500/15 border border-slate-700/80 hover:border-amber-500/50 text-slate-300 hover:text-amber-300 transition-all text-xs font-bold cursor-pointer group shadow-sm active:scale-95"
              ...
              {/* ELO Badge (cliquable également pour ouvrir le classement) */}
              <button
                onClick={() => setIsLeaderboardOpen(true)}
                className={`flex items-center space-x-2 px-3 py-1.5 rounded-full border text-xs font-bold shadow-inner cursor-pointer hover:scale-105 active:scale-95 transition-all ${getEloBadgeColor(user.elo)}`}
                title="Votre score ELO • Cliquer pour voir le classement"
              >
              ...
              <button
                ...
                className="flex items-center space-x-2.5 bg-dark-800/80 hover:bg-dark-700/80 border border-slate-800 hover:border-slate-700 px-3 py-1.5 rounded-full transition-all cursor-pointer"
